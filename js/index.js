...
var swiper = new Swiper(".mySwiper", {
  ...
  mousewheel: true, // 마우스 휠로 슬라이드 넘김을 활성화
});
...
var mySwiper = new Swiper("#swiper-container2", {
  ...
  on: {
    ...
    transitionStart: function () {
      const currentSlideHeight = $(".swiper-slide-active").height();
      const windowHeight = $(window).height();
      const containerTop = (windowHeight - currentSlideHeight) / 100;
      $(".swiper-wrapper").css(
        "transform",
        `translate3d(0, ${containerTop}px, 0)`
      );
...
const textspan = document.querySelectorAll(".define-count"); //container-9-sp라는 클래스를 가진 모든 요소를 선택하여 가져온다.
...
const spanAnimationOptions = {
  ...
  easing: "cubic-bezier(0.4, 0.0, 0.2, 1)",//애니메이션의 이징 함수
};
//Intersection Observer 콜백 함수
const intersectionCallback13 = (entries, observer13) => {
  const entry = entries[0]; //첫번째 entry만 사용한다.
  if (entry.isIntersecting) {
    //요소가 뷰포트에 진입한 경우
    setTimeout(() => {
      textspan.forEach((textsp, index) => {
        //선언한 변수를 순회하며 각 요소에 애니메이션을 적용한다.
        const delay = index * 500; // 요소마다 2초씩 지연 시간을 적용합니다. (변경 가능)
        setTimeout(() => {
          textsp.animate(spanKeyframes, spanAnimationOptions); //애니메이션 적용
        }, delay); //textlist 변수는 container-9-sp를 통해 선택된 클래스 요소를 나타내며 이 textlist에 포함된 각 요소들을 textsp라는 변수를 통해 순회하며 애니메이션을 적용한다
      });
    }, 200);

    observer13.unobserve(entry.target); //한번만 작동하도록 Intersection Observer을 해제한다
  }
};
//Intersection Observer 설정
const observer13 = new IntersectionObserver(intersectionCallback13, {
  root: null, //root 엘리먼트는 viewport
  rootMargin: "0px", //rootMargin 없음
  threshold: 0.5, //뷰포트의 50% 이상이 보이면 콜백호출
});
observer13.observe(document.querySelector(".define-img"));
...
//--------------------------------------------------------------------------------
// const textP = document.querySelectorAll(".container-p"); //container-9-sp라는 클래스를 가진 모든 요소를 선택하여 가져온다.

// // 키프레임 정의
// const txtrotateKeyframes = [
//   { opacity: 0 }, // 투명도가 0이고, 아래로 이동한 상태
//   { opacity: 1 },
// ];
// //애니메이션의 옵션을 설정한다
// const txtAnimationOptions = {
//   duration: 1600, //애니메이션 지속 시간
//   fill: "forwards", //애니메이션이 완료된 후 상태 유지
//   easing: "ease-in-out", //애니메이션의 이징 함수
// };
// //Intersection Observer 콜백 함수
// const intersectionCallback12 = (entries, observer12) => {
//   const entry = entries[0]; //첫번째 entry만 사용한다.
//   if (entry.isIntersecting) {
//     //요소가 뷰포트에 진입한 경우
//     setTimeout(() => {
//       textP.forEach((textsp, index) => {
//         //선언한 변수를 순회하며 각 요소에 애니메이션을 적용한다.
//         const delay = index * 900; // 요소마다 2초씩 지연 시간을 적용합니다. (변경 가능)
//         setTimeout(() => {
//           textsp.animate(txtrotateKeyframes, txtAnimationOptions); //애니메이션 적용
//         }, delay); //textlist 변수는 container-9-sp를 통해 선택된 클래스 요소를 나타내며 이 textlist에 포함된 각 요소들을 textsp라는 변수를 통해 순회하며 애니메이션을 적용한다
//       });
//     }, 500);

//     observer12.unobserve(entry.target); //한번만 작동하도록 Intersection Observer을 해제한다
//   }
// };
// //Intersection Observer 설정
// const observer12 = new IntersectionObserver(intersectionCallback12, {
//   root: null, //root 엘리먼트는 viewport
//   rootMargin: "0px", //rootMargin 없음
//   threshold: 0.5, //뷰포트의 50% 이상이 보이면 콜백호출
// });
// observer12.observe(document.querySelector(".container9-img"));




const textP = document.querySelectorAll(".container-p");
...
const intersectionCallback12 = (entries, observer12) => {
  ...
  if (entry.isIntersecting) {
    setTimeout(() => {
      textP.forEach((text) => {
        text.animate(txtrotateKeyframes, txtAnimationOptions);
      });
    }, 2000); // 5초 뒤에 애니메이션을 시작하도록 수정
